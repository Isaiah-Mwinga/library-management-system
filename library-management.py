...
if __name__ == "__main__":
    ...
    kabaraklibrary = Library(
        ["biology", "invention", "rich&poor", "computer", "macroeconomics", "microeconomics"])
    student = Student()
    track = []

    print("\t\t\t\t\t\t\t♦♦♦♦♦♦♦ WELCOME TO THE KABARAK LIBRARY ♦♦♦♦♦♦♦\n")
    ...
    while (True):
        ...
        try:
            ...
            if usr_response == 1:  # listing
                kabaraklibrary.displayAvailableBooks()
            elif usr_response == 2:  # borrow
                kabaraklibrary.borrowBook(
                    input("Enter your name: "), student.requestBook())
            elif usr_response == 3:  # return
                kabaraklibrary.returnBook(student.returnBook())
            elif usr_response == 4:  # donate
                kabaraklibrary.donateBook(student.donateBook())
